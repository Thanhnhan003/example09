...
export default function NewArrivals() {
    ...
    useEffect(() => {
        const fetchProductsByTagId = async () => {
            try {
                const tagId = 4; // Ví dụ: tagId là 1
                ...
                <div className="showcase-container" key={index}>
                    {chunk.map((product) => (
                        <div className="showcase" key={product.productId}>
                            <a href={`/detail/${product.productId}`} className="showcase-img-box">
                            ...
                            <div className="showcase-content">
                                ...
                                <a href="#" className="showcase-category">{product.product.productCategories[0]?.category.name}</a> {/* Lấy tên của danh mục sản phẩm */}
